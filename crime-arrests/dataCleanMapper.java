...
public class dataCleanMapper extends Mapper<LongWritable, Text, Text, Text> {
  ...
  private String[] geoLocs = {
        "10001,40.750633,-73.997177", //Manhattan
        ...
        "10041,40.7034,-74.0097",
        "10044,40.761915,-73.949962",
        "10045,40.7089,-74.0094",
        "10055,40.7590,-73.9742",
        "10060,40.751095,-73.994289",
        ...
        "10281,40.7111,-74.0146",
        "10282,40.716921,-74.015066", //Manhattan
        "10301,40.627456,-74.094407", //Staten 
        ...
        "10314,40.599263,-74.165748", //Staten
        "10451,40.820479,-73.925084", //Bronx
        ...
        "10475,40.875169,-73.823817", //Bronx
        "11101,40.747155,-73.939750", //Queens
        ...
        "11109,40.745115,-73.956928", //Queens
        "11201,40.693682,-73.989693", //Brooklyn
        ...
        "11239,40.647735,-73.879477", // Brooklyn
        "11241,40.6934,-73.9907", //Brooklyn added
        "11242,40.6931,-73.9908",
        "11243,40.6855,-73.9778",
        "11249,40.7119,-73.9661",
        "11252,40.6112,-74.0278",
        "11256,40.6635,-73.8604",// Brooklyn added
        "11351,40.780747,-73.825301", // Queens
        ...
        "11697,40.555688,-73.920663", // Queens
    };
  ...
  public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {  
    ...
    String arrest_code = tokens[4];
    String arrest_description = tokens[5]; 
    String arrest_level = tokens[7];
    
    // Remove the malformed rows 
    if (arrest_date.isEmpty() || arrest_code.isEmpty() || arrest_description.isEmpty() || arrest_level.isEmpty() || !isNum(tokens[16]) || !isNum(tokens[17])) {
      return;
    }

    // Remove the row that is reported after 2017
    if (Integer.parseInt(arrest_date.substring(arrest_date.length() - 2)) > 17){
      return;
    }
    
    double latitude = Double.parseDouble(tokens[16]);
    double longitude = Double.parseDouble(tokens[17]);

    double curr_minDistance = Double.POSITIVE_INFINITY;
    String curr_minZipcode = "";

    for (String geo : this.geoLocs) {
      String[] zip_lat_lon = geo.split(",");
      String zipcode = zip_lat_lon[0];
      double lat = Double.parseDouble(zip_lat_lon[1]);
      double lon = Double.parseDouble(zip_lat_lon[2]);

      // Calculate the closest coordinate
      double distance = Math.sqrt(Math.pow((longitude - lon), 2) - Math.pow((latitude - lat), 2));
      
      if (curr_minDistance > distance) {
        curr_minDistance = distance;
        curr_minZipcode = zipcode;
      }
    }

    int zip_int = 0;
    if(curr_minZipcode.length() != 0 ){
      zip_int = Integer.parseInt(curr_minZipcode);
    }
    String borough = "None";

    if (zip_int <= 10282){
      borough = "Manhattan";
    }else if( zip_int > 10282 && zip_int <= 10314){
      borough = "Staten Island";
    }else if(zip_int > 10314 && zip_int <= 10475){
      borough = "Bronx";
    }else if ((zip_int > 10475 && zip_int <= 11109) || (zip_int >= 10351 && zip_int <= 11697)){
      borough = "Queens";
    }else if(zip_int > 11109 && zip_int <= 11256){
      borough = "Brooklyn";
    }
   
    context.write(new Text(id), new Text(curr_minZipcode + '|' + arrest_date + '|' + arrest_code + '|' + arrest_description + '|' + arrest_level + '|' + borough));
  }
}